...
(define (merge-table cond tbl1 tbl2)
  (append (map (lambda (r) (cons (car r) (if cond (cdr r) 0))) tbl1)
          (map (lambda (r) (cons (car r) (if cond 0 (cdr r)))) tbl2)))
  ...
  (cond
    ...
    [else 
      (let* ([elev (car table)]
             ...
             [mult (cdr elev)])
        (cons
          (cons ele (if (eq? mult 0) 0 1))
          (dedup (map (lambda (r) (cons (car r) (if (&& (not (eq? mult 0)) (equal? (car r) ele)) 0 (cdr r)))) (cdr table)))))]))
      ...
      (let ([ele (car table)])
        (cons 
          (cons (car ele)
                ...
                (foldl + 0 (map cdr (map (lambda (x) (cons (car x) (if (equal? (car ele) (car x)) (cdr x) 0))) table))))
          (dedup-accum 
            ;(filter (lambda (x) (not (equal? (car ele) (car x))))
            (map (lambda (x) (cons (car x) (if (equal? (car ele) (car x)) 0 (cdr x))))
                    (cdr table)))))]))
